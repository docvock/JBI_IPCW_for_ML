###################################
# File: ./IPCW_analysis.R
#		Purpose: Use the machine learning techniques (logistic regression, GAM, recursive partitioning,
#							k-NN) to estimating probability of having a cardiovascular adverse event within 5 years.
#							Four different techniques to handle observations for which the event status cannot be 
#							determined are used including 1) discarding the observations, 2) treating those observations
#							as if the event did not occur, 3) splitting the observations into two, one where E=1 and the 
#							other with E=0 and 4) using IPCW. The file also outputs the tables for the calibration and 
#							discrimination metrics.
#   Author: David M. Vock
#   Last Modified: January 8, 2016
#   Relies On: ./IPCW_fncs.R 
#								./model-evaluation-metrics.R
#								./Data_ver-2.3.1_imputed.csv
#   Files Created: ./calibration_plots.pdf 
#										./calibration_plots.eps
####################################
...
pRP <- rpart_functions(varnms, "IPCW", train.dat = HP.train, test.dat = HP.test, myseed = 1101985,
                       ...
                       split = "information", risk.cutpts = risk.cutpts, cp=1e-4, verbose = TRUE)
